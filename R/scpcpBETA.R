...
scpcpBETA<-function(data, freqvar = "Freq",
                gap = 0.2,
                ...
                col.opt = list(),
                plot = TRUE,
	            return.coords = !plot, numeric = NULL)
{
 ...
 orig.nm <- names(data)
  ...
  if(!is.null(numeric)){
 	# get indices of all numeric variables
 	 if(numeric[1] < 1){
 		numeric <- sapply(data,function(z){
 		if(is.factor(z) || is.character(z)){
 			return(FALSE)
 		} 
 		ret <- any( abs(z)%%1 > 0) || length(table(z)) > numeric*nrow(data)
 		return(ret)
 	})
 	numeric[ which(names(data) == "Freq")] <- FALSE
 	if(any(numeric)){
 		numeric <- which(numeric)
 	}else{
 		numeric <- NULL
 	}
 }
...
if(!is.null(numeric)){
	# numeric and factors are indices for data
	# they change if a selection is added as an additional variable
  		num.data <- apply(data[,numeric,drop = FALSE],2,function(z){
  			z <- z - min(z)
  			z <- z/max(z)
  		})
  		factors <- c(1:ncol(data))[-numeric]
  		data <- data[,-numeric]
  		#polygon <- FALSE
  		#numeric <- numeric + (1 - sel.hide)
...
############## PARAM 2 #############
 ...
 if(!is.null(numeric)){
    	num.data <- num.data[base.order,,drop = FALSE]
   }
  ...
  if(sort.individual & !doodle){  # Sortiert jede Sequenz nach den R?ngen der jeweils linken Variable
    ...
    for(i in 2:m){
    	if(factors[i] == (1+factors[i-1])){
    		...
    		# the left variable is numeric and has to be taken from num.data
    		ii <- which(numeric == factors[i]-1)
    		sapply(id.list[[i]], function(z){
    			rr <- rank(num.data[z,ii],ties.method="first")
        		e1$lines[z,i]  <- lines[z,i][rr] 
        		return(invisible(TRUE))
      		})
    	}
    ...
    for(i in 2:m){
    	if(factors[i] == 1+factors[i-1]){
    		...
    		sapply(id.list[[i]], function(z){
        		tapply(z,e1$data[z,1],function(y){
        			e1$lines[y,i]  <- lines[y,i][rank(e1$num.data[y,ii],ties.method="first")]
        		})
    			return(invisible(TRUE))
      		})     
    	}
...
if(plot){
  ...
  if(!polygon){
    ...
    if(!is.null(numeric)){
    	# set m to original value
    	m <- m+length(numeric)
    	
    	# original categorical indices
    	ff <- rep( (1:m)[-numeric], each = 2)
    	
    	# numeric indices in expanded matrix lines.doubled
    	nn <- 2*(numeric) - (rank(numeric))
       	
    	lines.doubled <- matrix(0,nrow(lines), 2*m - length(numeric))

    
    	lines.doubled[,-nn] <- lines[,rep( 1:(m - length(numeric)) ,each=2)] # Achsen verdoppeln um Platz f?r Beschriftung zu schaffen
    	lines.doubled[,nn] <-  num.data
    	
    	
    	
    	lines2 <- matrix(0,nrow(lines),m)
    	lines2[,numeric] <- num.data
    	lines2[,-numeric] <- lines
    	lines <- lines2
    	...
    	for (i in 1:m){
    		if(i %in% ff){
    			...
    			xcoords <- c(xcoords,i+level.width/2)
    		}
 ...
 if(!is.null(numeric)){
 	xx <- 0.5*level.width + (1:m)[-numeric]
 }else{
...
zz <- unlist(ll)
 ...
 if(!is.null(numeric)){
 	nm2 <- 1:m
 	# if doodle == FALSE, then sel.hide was set to FALSE
 	# if doodle == TRUE we need to take sel.hide into account:
 	nm2[numeric] <- orig.nm[numeric - 1 + (sel.hide==doodle)]
 	nm2[-numeric] <- nm
 	nm <- nm2
 	rm(nm2)
 }
